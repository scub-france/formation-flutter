...
// Contrairement à un contructeur, une méthode de fabrique (factory method) peut retourner une instance de n'importe quelle sous-classe ou implémentation de l'interface.
class MyClass {
  late final int monChamp; // Ici, nous avons une classe avec un champs qui ne peut être modifié après la création de l'objet

  MyClass._(this.monChamp); // constructeur privé qui ne peut être appelé depuis l'intérieur de la classe.

  factory MyClass.create(int value) { // Factory nommé `create()` qui prend un entier et retourne une instance de `MyClass`
    return MyClass._(value * 2); // Retourne la valeur multiplié par 2 et utilise un constructeur privé pour créer l'instance de `MyClass`
  }
}

void main8() {
  var instance = MyClass.create(7); // Nous appelons factory method `create()` pour créer une instance de `MyClass` avec la valeur d'entrée 7.
  print(instance.monChamp); // factory method retourne une instance de `MyClass` avec `monChamp` égale à 14, que nous stockons dans la variable `instance`. Nous pouvons ensuite accéder à la valeur de `monChamp` en utilisant la syntaxe `instance.monChamp`.
...
// Nous voulons, pour cette exemple, accéder à la concaténation de 2 propriétés privées
class Passenger {
  String _firstName;
  String _lastName;

  Passenger(this._firstName, this._lastName);

  // Champ calculé `fullName` est une propriété calculée dynamiquement à partir de deux autres propriétés, plutot qu'un champ de données qui stocke directement une valeur
